...
// All correct answers
const answers = {
    'q1': ['put the bag on the table'],
    'q2': ['put the flower vase on the table', 'put the vase on the table'],
    'q3': ['put the pencil on the table'],
    'q4': ['put the shoes under the table'],
    'q5': ['put the sandals under the table', 'put the sandal under the table'],
    'q6': ['put the pencil case in the bag'],
    'q7': ['put the book in the bag'],
    'q8': ['put the umbrella in the bag'],
    'q9': ['hang the jacket on the hook'],
    'q10': ['hang the umbrella on the hook']
};
...
function playClickSound() {
    const snd = document.getElementById('audio-typing');
    if (snd) { snd.currentTime = 0; snd.play().catch(e => {}); }
}

function playCorrectSound() {
    const snd = document.getElementById('audio-correct');
    if (snd) { snd.currentTime = 0; snd.play().catch(e => {}); }
}

function playWrongSound() {
    const snd = document.getElementById('audio-incorrect');
    if (snd) { snd.currentTime = 0; snd.play().catch(e => {}); }
}
...
async function checkAnswers() {
    ...
    for (let i = 1; i <= TOTAL_QUESTIONS; i++) {
        ...
        const row = zone.closest('.content-row');
        if (row) {
