document.addEventListener('DOMContentLoaded', () => {
    ...
    // Image Zoom Modal
    const exampleZoomImg = document.getElementById('example-zoom-img');
    const sec2ZoomImg = document.getElementById('sec2-zoom-img');
    const imageModal = document.getElementById('image-modal');
    const zoomedImage = document.getElementById('zoomed-image');
    const closeModal = document.getElementById('close-modal');
    const fabViewImage = document.getElementById('fab-view-image');
    const fabContainer = document.querySelector('.fab-container');

    if (imageModal && zoomedImage && closeModal) {
        ...
        if (exampleZoomImg) {
            exampleZoomImg.addEventListener('click', () => openImageModal(exampleZoomImg.src));
        }

        if (sec2ZoomImg) {
            sec2ZoomImg.addEventListener('click', () => openImageModal(sec2ZoomImg.src));
        }

        if (fabViewImage && exampleZoomImg) {
            fabViewImage.addEventListener('click', () => openImageModal(exampleZoomImg.src));
        }
        ...
        if (fabContainer) {
            window.addEventListener('scroll', () => {
                ...
                if (exampleSection && questionsGrid) {
                    const exRect = exampleSection.getBoundingClientRect();
                    const gridRect = questionsGrid.getBoundingClientRect();
                    
                    // Show when example is scrolled out AND grid is still partially visible
                    const isExampleHidden = exRect.bottom < 0;
                    const isGridVisible = gridRect.bottom > 50; 
                    
                    if (isExampleHidden && isGridVisible) {
    ...
    checkBtn.addEventListener('click', async () => {
        ...
        // Sheets URL: https://docs.google.com/spreadsheets/d/1qQBJAuxZ3WX03hyFQR4Emowb3y9G1S-c9MyWyJ-cn-k/edit?usp=sharing
        ...
        const formUrl = 'https://docs.google.com/forms/d/e/1FAIpQLSd46u5EUNv56riOBaAjqd1AWxJ4EhbaE8o6Yt_hXIX3jko4GA/formResponse';

        const formParams = new URLSearchParams();
        formParams.append('entry.1874922071', studentName);
        formParams.append('entry.2074478787', schoolName);
        formParams.append('entry.2079793092', finalScore);
        formParams.append('entry.1410544823', 'Formative Assessment Lesson 4');
