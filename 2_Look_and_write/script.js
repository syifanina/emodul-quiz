...
document.addEventListener('DOMContentLoaded', function () {
    ...
    inputs.forEach(input => {
        ...
        hint.style.textTransform = 'lowercase';
    ...
    const gifContainers = document.querySelectorAll('.img-placeholder, .intro-image-container');
    
    gifContainers.forEach(container => {
        const img = container.querySelector('img');
        if (img) {
            // Check if it already has an hourglass icon so we don't duplicate
            if (!container.querySelector('.loading-icon')) {
                const iconDiv = document.createElement('div');
                iconDiv.className = 'loading-icon';
                iconDiv.innerHTML = `<svg viewBox="0 0 24 24"><path d="${loadingIconPath}"></path></svg>`;
                container.appendChild(iconDiv);
            }
            
            // Add loading text overlay delay (300ms)
            ...
            if (img.complete) {
                ...
                img.addEventListener('load', onLoadOrComplete);
            }
        }
    });
});
...
async function checkAnswers() {
    ...
    for (let i = 1; i <= TOTAL_QUESTIONS; i++) {
        ...
        const expectedAnswers = (zone.dataset.answer || '').toLowerCase().trim().split('|');
        const placed = zone.value.toLowerCase().trim();
        ...
        if (expectedAnswers.includes(placed)) {
            ...
            if (hint) { hint.textContent = expectedAnswers[0]; hint.style.display = 'block'; }
            playWrongSound();
        }
