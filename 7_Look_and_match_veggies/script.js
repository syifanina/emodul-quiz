document.addEventListener('DOMContentLoaded', () => {
    ...
    let connections = {}; // format: { '1': { line: svgElement, type: 'veggie', element: DOMElement } }

    const totalVeggies = Array.from(items).filter(i => i.dataset.type === 'veggie').length;
    ...
    function updateCheckButtonState() {
        ...
        // Tombol hanya aktif jika jumlah koneksi sudah sama atau lebih dari total sayuran (termasuk contoh)
        if (connIds.length >= totalVeggies) {
    ...
    checkBtn.addEventListener('click', async () => {
        ...
        // Check if user missed any veggies
        let missedCount = 0;
        let missedLines = [];
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            if (item.dataset.type === 'veggie') {
                const id = item.dataset.id;
                if (!connections[id]) {
                    // User missed this veggie!
                    await new Promise(resolve => setTimeout(resolve, 600));
                    ...
                    playSound(correctSound); // Play correct sound since it's a veggie
        ...
        if (correctVeggieCount === totalVeggies && !anyWrong) {
            resultMsg.textContent = "Perfect! What a delicious vegetable soup!";
            resultMsg.style.color = "#16a34a";
            playSound(correctSound);
        } else if (!anyWrong && correctVeggieCount < totalVeggies) {
            resultMsg.textContent = "Good start, but you missed some vegetables!";
            resultMsg.style.color = "#2563eb";
        } else {
            resultMsg.textContent = "Oops! You added some fruits. Try again!";
