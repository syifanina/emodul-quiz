document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Helper to normalize text input
     */
    ...
    /**
     * Update Word Bank: Cross out typed words
     */
    const wordBankCells = document.querySelectorAll('.word-bank-table td:not(.example-bank)');
    function updateWordBank() {
        const typedWords = new Set();
        sec3Inputs.forEach(input => {
            const val = normalizeStr(input.value);
            if (val) typedWords.add(val);
        });

        wordBankCells.forEach(cell => {
            const cellWord = normalizeStr(cell.textContent);
            if (typedWords.has(cellWord)) {
                cell.classList.add('used-word');
            } else {
                cell.classList.remove('used-word');
            }
        });
    }
    ...
    checkBtn.addEventListener('click', async () => {
        ...
        const pointPerInput = sec3Inputs.length > 0 ? (60 / sec3Inputs.length) : 0;
        ...
        for (let i = 0; i < sec3Inputs.length; i++) {
            ...
            // don't hardcode a single string for display answer in hint if they can choose from 11 options
            const isFruit = correctAnswers.includes('strawberry');
            const displayAnswer = isFruit ? 'Any unused fruit' : 'Any unused vegetable';
            ...
            if (correctAnswers.includes(userAnswer)) {
                ...
                if (hint) {
                    hint.textContent = `Check word bank`;
                    hint.style.display = 'block';
                }
                playSound(incorrectAudio);
            }

            // slightly faster animation so kids don't wait forever for 23 inputs
            await new Promise(resolve => setTimeout(resolve, 300));
        }

        // Calculate and Show Total Score
        const totalPoints = pgPoints + sec2Points + sec3Points; // Max: 40 + 0 + 60 = 100
        const finalScore = Math.round(totalPoints); // Out of 100
        ...
        // Sheets URL: https://docs.google.com/spreadsheets/d/1sSeLjXlKynRtb3INE_s6VrkpObvysNOP_2SFl_a-x3Y/edit?usp=sharing
        ...
        const formUrl = 'https://docs.google.com/forms/d/e/1FAIpQLSehY4eO-05nSQ3M58Vbg8zqnERkwTXWaznCF2tlBv3DLaDMtA/formResponse';

        const formParams = new URLSearchParams();
        formParams.append('entry.1502147898', studentName);
        formParams.append('entry.781991223', schoolName);
        formParams.append('entry.814727243', finalScore);
        formParams.append('entry.899196606', 'Formative Assessment Lesson 7');
